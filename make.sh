#!/usr/bin/env -S zsh --no-rcs --errexit
...
if [[ $# -gt 0 ]]; then
	...
	if [[ $1 == 'rm' ]]; then
		...
		$RM $BLDDIR $CDCACHE $NINJAFILE $RLSDIR
		exit 0
...
function generate_ninja {
	...
	if [[ $MODE == 'release' ]]; then
		...
		echo "  command = \$archiver -rcs xns/\$out \$in" >> $NINJAFILE
		echo "  description = archiving \$out\n" >> $NINJAFILE

		# single header generation
		echo "rule single_header" >> $NINJAFILE
		echo "  command = $SCRIPTDIR/single_header.sh" >> $NINJAFILE
