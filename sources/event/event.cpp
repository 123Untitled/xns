...
//: _input{}, _buff{}, _readed{0}, _is_running{false},
//	_windowed{false},
...
//	md._state = false;
//	// remove mode
//	//_modes.erase(md._idx); // temporary comment for avoid new vector errors
//}
//
///* set mode */
...
//
//	// set running flag
//	shared()._is_running = true;
//
//	// loop over reading
//	while (shared()._is_running) {
...
//		shared()._input.append(shared()._buff, static_cast<size_type>(shared()._readed));
//
//		// buffer is full, loop over reading
//		if (shared()._readed == BUFFER_SIZE) {
//			// read stdin again
//			while (read_stdin() > 0) {
//				// append buffer to the string
//				shared()._input.append(shared()._buff, static_cast<size_type>(shared()._readed));
...
//	shared()._readed = read(STDIN_FILENO, shared()._buff, BUFFER_SIZE);
//	// append null terminator
//	shared()._buff[shared()._readed] = '\0';
//	// return number of readed bytes
//	return shared()._readed;
//}
//
//
//void xns::event::dispatch(void) {
//
//	if (shared()._input.empty()) { return; }
//
//	// get event instance
//	xns::event& evnt = xns::event::shared();
//
...
//
//
//	if (shared()._input.length() == 3 && shared()._input[0] == 0x1b) {
//
//		// check ctrl sequence
//		if (shared()._input[1] == 0x5b) {
//			// check arrow sequence
//			switch (shared()._input[2]) {
...
//	}
//
//	if (shared()._input.length() == 1) {
//		const xns::ubyte _char = (xns::ubyte)shared()._input[0]; // NEED TO FIX CONVERSION
